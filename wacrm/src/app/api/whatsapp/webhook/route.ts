  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      try {
        ...
        console.error(
          '[webhook] Token decryption failed for phone_number_id:',
          phoneNumberId,
          err,
        )
        continue
...
async function handleStatusUpdate(status: {
  ...
  const recipient = await prisma.broadcastRecipient
    .findFirst({
  ...
  await prisma.broadcastRecipient
    .update({
      ...
      data: update as { status: string; sentAt?: string; deliveredAt?: string; readAt?: string },
    })
    .catch((recUpdateErr) => {
...
async function flagBroadcastReplyIfAny(userId: string, contactId: string) {
  try {
    ...
    await prisma.broadcastRecipient.update({
      where: { id: row.id },
      data: { status: 'replied', repliedAt: new Date().toISOString() },
    })
  } catch (err) {
...
async function lookupInternalIdByMetaId(
  ...
  const data = await prisma.message.findFirst({
    where: { messageId: metaId, conversationId },
    select: { id: true },
  })
  return data?.id ?? null
}
...
async function handleReaction(
  ...
  if (!reaction.emoji) {
    await prisma.messageReaction
      ...
      .catch((e) => {
        console.error('[webhook] reaction delete failed:', e.message)
      })
    return
  }
  ...
  await prisma.messageReaction
    ...
    .catch((e) => {
      console.error('[webhook] reaction upsert failed:', e.message)
    })
}
...
async function processMessage(
  ...
  const createdMessage = await prisma.message
    .create({
      data: {
        conversationId: conversation.id,
        senderType: 'customer',
