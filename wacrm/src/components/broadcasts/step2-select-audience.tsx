...
import { ContactTag, CustomField, Tag } from '@/types';
  ...
  const fetchEstimatedCount = useCallback(async () => {
    ...
    try {
      ...
      if (audience.type === 'all') {
        ...
        baseIds = new Set((data ?? []).map((r: ContactTag) => r.contact_id));
      } else if (
      ...
      if (audience.excludeTagIds && audience.excludeTagIds.length > 0) {
        ...
        excludeSet = new Set((excludeRows ?? []).map((r: ContactTag) => r.contact_id));
      }
