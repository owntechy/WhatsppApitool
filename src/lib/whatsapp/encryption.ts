...
export function decrypt(encryptedText: string): string {
  ...
  if (parts.length === 2) {
    ...
    if (iv.length !== CBC_IV_LENGTH) {
      if (iv.length === GCM_IV_LENGTH) {
        throw new Error(
          `Stored token appears to be a corrupted GCM ciphertext (missing auth tag, found 2-part format with 12-byte IV). ` +
          `This usually means the ENCRYPTION_KEY changed or the DB value was truncated. ` +
