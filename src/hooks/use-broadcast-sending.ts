...
import { BroadcastRecipient, Contact, ContactCustomValue, ContactTag, MessageTemplate } from '@/types';
...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function resolveAudience(audience: AudienceConfig): Promise<Contact[]> {
    ...
    if (audience.type === 'all') {
      ...
      if (contactTags && contactTags.length > 0) {
        const uniqueContactIds = [
          ...new Set(contactTags.map((ct: ContactTag) => ct.contact_id)),
        ];
    ...
    if (audience.excludeTagIds && audience.excludeTagIds.length > 0) {
      ...
      const excludedIds = new Set((excludeRows ?? []).map((r: ContactTag) => r.contact_id));
  ...
  async function resolveCustomFieldAudience(
    ...
    const contactIds = [...new Set((matches ?? []).map((m: ContactCustomValue) => m.contact_id))];
